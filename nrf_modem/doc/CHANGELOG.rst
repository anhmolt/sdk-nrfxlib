...
* Fixed:
  ...
  * A bug where setting the value of the :c:macro:`NRF_SO_SEC_HOSTNAME` socket option could led to a crash if the value had been previously cleared using :c:func:`nrf_setsockopt` and setting ``option_len`` equal to zero.
...
* Updated:
  ...
  * The ``data`` parameter of :c:func:`nrf_modem_softsim_res` with the `const` data type qualifier because the data it is not modified by the function.
